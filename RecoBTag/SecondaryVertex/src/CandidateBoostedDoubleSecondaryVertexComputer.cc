...
float CandidateBoostedDoubleSecondaryVertexComputer::discriminator(const TagInfoHelper & tagInfo) const
{
  ...
  /* std::vector<const reco::BaseTagInfo*>  baseTagInfos;
   ...
   const reco::BaseTagInfo * ipBaseTagInfo =  &tagInfo.getBase(0); 
   baseTagInfos.push_back( ipBaseTagInfo );
   const reco::BaseTagInfo * svBaseTagInfo =  &tagInfo.getBase(1);
   baseTagInfos.push_back( svBaseTagInfo );
   */
  ...
  // default variable values
   ...
   int tau2_nSecondaryVertices =0, tau1_nSecondaryVertices=0, jetNTracks=0;
   ...
   size_t trackSize = selectedTracks.size();
        ...
        for (size_t i =0; i<indices.size(); ++i)
        {
               ...
               && !charmThreshSet )
          {
            ...
            //if ( (i+1)<indices.size() ) trackSip2dSigAboveCharm_1 = (ipData[indices[i+1]]).ip2d.significance();
     ...
     if (track_PVweight>0.) { allKinematics.add(ptrack, track_PVweight); }
        ...
        for (unsigned int jtt=0; jtt < trackSize; ++jtt)
        {
         ...
         GlobalVector direction(jet->px(), jet->py(), jet->pz());
          ...
          //if (closest.isValid()) lengthTau =  (closest.globalPosition() - RecoVertex::convertPos(vertexRef->position())).mag();

              //distTau = std::abs(IPTools::jetTrackDistance(transientTrack, direction, *vertexRef ).second.value());

	float IPsig= ipTagInfo.impactParameterData()[itt].ip3d.significance();
          if( isfromV0==0 && lengthTau<5. && distTau<0.07)
        {
          IP3Ds.push_back(         IPsig<-50. ? -50. :         IPsig );
          contTrk++;
         if (currentAxes.size() > 1)
          {
            if (reco::deltaR2(ptrack,currentAxes[0]) < reco::deltaR2(ptrack,currentAxes[1]))
              IP3Ds_1.push_back(        IPsig        <-50. ? -50. :         IPsig        );
            else
              IP3Ds_2.push_back(        IPsig        <-50. ? -50. :         IPsig        );
          }
          else
            IP3Ds_1.push_back(        IPsig        <-50. ? -50. :         IPsig        );
        }
...
// 
   ...
   for (size_t vtx = 0; vtx < svTagInfo.nVertices(); ++vtx)
   {
    ...
    vertexKinematics(vertex, vertexKinematic);
     ...
     {
       ++contSV;
       VTXmass[svTagInfo.flightDistance(vtx).error()]=vtx;
     }
   }
...
if( (tau1_vertexMass<0 && tau2_vertexMass>0) )
      {
        ...
        temp = tau1_nSecondaryVertices;
        tau1_nSecondaryVertices= tau2_nSecondaryVertices;
        tau2_nSecondaryVertices= temp;
   ...
   for ( std::map<double, size_t>::iterator iVtx=VTXmass.begin(); iVtx!=VTXmass.end(); ++iVtx)
   {
     ++cont;
...
void CandidateBoostedDoubleSecondaryVertexComputer::etaRelToTauAxis(const reco::VertexCompositePtrCandidate & vertex, fastjet::PseudoJet & tauAxis, std::vector<float> & tau_trackEtaRel) const
{
